...
Scene::Scene() 
    : m_resourceManager{ ResourceManager() }, m_I{ 0.6f, 0.6f, 0.6f, 1.0f }, m_ambientAlbedo{ 0.6f, 0.6f, 0.6f, 1.0f },
...
void Scene::SetUpScene() {
    ...
    auto& cubeMesh = m_resourceManager.GetMesh(MeshID::CUBE);
    m_objects.emplace_back(cubeMesh, ImageID::STONE_TEX, Translate(0, BASE_POS_Y, 0) * Scale(baseSize));

    auto& vaseMesh = m_resourceManager.GetMesh(MeshID::VASE);
    ...
    auto& planeMesh = m_resourceManager.GetMesh(MeshID::PLANE);
    ...
    auto& sphereMesh = m_resourceManager.GetMesh(MeshID::SPHERE);
